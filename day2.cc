...
void ProcessFilePart1(const std::string& filename);
void ProcessFilePart2(const std::string& filename);

int main(int argc, char* argv[]) {
  absl::ParseCommandLine(argc, argv);
  std::string file_path = absl::GetFlag(FLAGS_input_file);
  ProcessFilePart2(file_path);
}
...
void ProcessFilePart2(const std::string& filename) {
  ...
  while (std::getline(file, line)) {
    ...
    sum += green_count * blue_count * red_count;
